...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  cardMemberIds = cardMemberIds.map(_ => _.memberId)

  const board = useSelector(selectCurrentActiveBoard)

  const FE_CardMembers = cardMemberIds.map(memberId => board.FE_allUsers.find(user => user.id === memberId))

  const handleUpdateCardMembers = (user) => {
    const incomingUserInfo = {
      userId: user.id,
      action: cardMemberIds.includes(user.id) ? CARD_MEMBER_ACTIONS.REMOVE : CARD_MEMBER_ACTIONS.ADD
    }
